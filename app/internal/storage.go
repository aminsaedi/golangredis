...
func AddEntryToStream(streamKey string, entryId string, dataItems []DataItem) (ok bool, err error) {
	si, exists := streamStorage[streamKey]
	if exists {
		// si.addEntry(entryId, dataItems)
		ok, err := si.addEntry(entryId, dataItems)
		streamStorage[streamKey] = si
		return ok, err
	}
